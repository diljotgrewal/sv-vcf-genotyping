...
def write_lumpy_output(lumpy_vcf, ids, output_file):
    input_open = gzip.open if lumpy_vcf.endswith('gz') else open
    output_open = gzip.open if output_file.endswith('gz') else open

    with output_open(output_file, 'wt') as writer, input_open(lumpy_vcf, 'rt') as reader:
        for line in reader:
            if line.startswith('#'):
                writer.write(line)
                continue

            brk_id = line.strip().split('\t')[2].split('_')[0] + '_lumpy'

            if brk_id in ids:
                writer.write(line)


def main(destruct_csv, lumpy_vcf, consensus_vcf):
    allcalls = [
    ...
    for _, brkgrp in allcalls:
        ...
        brk_id = [v for v in list(brkgrp['breakpoint_id']) if 'lumpy' in v]
        brk_id = brk_id[0]

        brkgrp = brkgrp[:1]
        brkgrp['breakpoint_id'] = brk_id

        outdata.append(brkgrp)

    outdata = pd.concat(outdata)

    breakpoint_ids = set(outdata['breakpoint_id'])

    write_lumpy_output(lumpy_vcf, breakpoint_ids, consensus_vcf)
...
def run(
    ...
    main(destruct_csv, lumpy_vcf, consensus_lumpy)
